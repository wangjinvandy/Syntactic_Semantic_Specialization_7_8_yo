...
writefile=[Tvaluefolder '_correlation2.txt'];
cd(root);
if exist(writefile)
   delete(writefile);
end
...
for i=1:length(subjects)
    txt_run1Finite=[root '/' Tvaluefolder '/Finite_vs_PC_run1/' num2str(subjects(i)) '/' ROI '_Finite_vs_PC_run1_output.txt'];
    fid=fopen(txt_run1Finite);
    data1=textscan(fid,'%d %d %d %f');
    finiterun1=data1{4};
    txt_run2Finite=[root '/' Tvaluefolder '/Finite_vs_PC_run2/' num2str(subjects(i)) '/' ROI '_Finite_vs_PC_run2_output.txt'];
    ...
    txt_run1InCon=[root '/' Tvaluefolder '/InCon_vs_PC_run1/' num2str(subjects(i)) '/' ROI '_InCon_vs_PC_run1_output.txt'];
    fid3=fopen(txt_run1InCon);
    data3=textscan(fid3,'%d %d %d %f');
    inconrun1=data3{4};
    txt_run2InCon=[root '/' Tvaluefolder '/InCon_vs_PC_run2/' num2str(subjects(i)) '/' ROI '_InCon_vs_PC_run2_output.txt'];
